    ...
    async (req, res) => {
      try {
        ...
        res.json({
          ...
          message:
            "Compte professionnel créé avec succès. En attente de vérification.",
        });
      } catch (error) {
        console.error("❌ Erreur création compte professionnel:", error);
        res.status(500).json({
          error: "Erreur serveur lors de la création du compte professionnel",
        });
      }
    },
  );
